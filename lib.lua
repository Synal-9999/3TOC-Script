...
function SaveManager:MarkChanged()
    self.LastChange = tick()
end

function SaveManager:Save(nf)
    ...
    local success = pcall(function()
        ...
        writefile(self.ConfigFolder .. "/" .. nf, encoded)
    end)
    if success then end
    return success
end
...
function SaveManager:GetConfigs()
    ...
    for _, file in pairs(listfiles(self.ConfigFolder)) do
        local nf = file:match("([^/\\]+)$")
        if nf:match("%.json$") then
            table.insert(configs, nf)
        end
    end
    return configs
end

function SaveManager:Toggle(enabled)
	...
	function Window:MakeTab(paste, Configs)
		...
		function Tab:AddTextBox(Configs)
		    ...
		    local Callback = Funcs:GetCallback(Configs, 4)
  		    ...
  		    if type(TDefault) ~= "string" or TDefault:gsub(" ", ""):len() < 1 then TDefault = "" end
  		    local Button, LabelFunc = ButtonFrame(Container, TName, TDesc, UDim2.new(1, -38))
            ...
            function TextBox:Callback(...) Funcs:InsertCallback(Callback, ...)(TextBoxInput.Text) end
            return TextBox
        end
